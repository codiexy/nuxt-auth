...
async function initCsrfCookie(
  ...
  if (config.endpoints.csrf === undefined) {
    throw new Error('`codiexyAuth.endpoints.csrf` is not defined')
  }
...
async function useCsrfHeader(
  ...
  if (config.csrf.cookie === undefined) {
    throw new Error('`codiexyAuth.csrf.cookie` is not defined')
  }

  if (config.csrf.header === undefined) {
    throw new Error('`codiexyAuth.csrf.header` is not defined')
  }
